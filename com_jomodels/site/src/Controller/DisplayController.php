...
/**
 * Jomodels master site display controller.
 *
 * @since   4.0
 */
class DisplayController extends BaseController
{
    ...
    function display($cachable = false, $urlparams = false)
    {
        ...
        if ($isEdit && !$this->checkEditId($view, $id))
        {
            ...
            if ($refid > 0 && StringHelper::check($ref))
            {
                ...
                $this->setRedirect(Route::_('index.php?option=com_jomodels&view=' . (string) $ref . '&layout=edit&id=' . (int) $refid, false));
            }
            elseif (StringHelper::check($ref))
            {
                // redirect to ref
                 $this->setRedirect(Route::_('index.php?option=com_jomodels&view=' . (string) $ref, false));
            }
            else
            {
                // normal redirect back to the list default site view
                $this->setRedirect(Route::_('index.php?option=com_jomodels&view=', false));
            }
    ...
    {
        if (parent::checkEditId("com_jomodels.edit.{$context}", $id))
        {
            return true;
        }
        ...
        {
            $this->holdEditId("com_jomodels.edit.{$context}", $id);
    ...
    protected function canEditId($view, $id): bool
    {
        ...
        {
            $access = ($this->identity->authorise($allowed['access'], "com_jomodels.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['access'], 'com_jomodels'));
        }
        ...
        {
            $edit = ($this->identity->authorise($allowed['edit'], "com_jomodels.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit'], 'com_jomodels'));
        }

        // check if this item can be edited by owner
        if ($access && !$edit && isset($allowed['edit.own']))
        {
            $edit = ($this->identity->authorise($allowed['edit.own'], "com_jomodels.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit.own'], 'com_jomodels'));
        }
